 ...
 * @property ?float $_470 Чистая прибыль (убыток)
 * @property ?float $_480 Целевое финансирование
...
class AccountingBalance extends ApiFetchedActiveRecord implements FinancialReportInterface
{
    ...
    private static function financialAttributes(): array
    {
        return [
            'ДОЛГОСРОЧНЫЕ АКТИВЫ' => [
                '_110' => 'Основные средства, т.р.',
                '_120' => 'Нематериальные активы, т.р.',
                '_130' => 'Доходные вложения в материальные активы:, т.р.',
                '_131' => 'в том числе: инвестиционная недвижимость, т.р.',
                '_140' => 'Вложения в долгосрочные активы, т.р.',
                '_150' => 'Долгосрочные финансовые вложения, т.р.',
                '_160' => 'Вложения в долгосрочные активы, т.р.',
                '_170' => 'Долгосрочная дебиторская задолженность, т.р.',
                '_180' => 'Прочие долгосрочные активы, т.р.',
                '_190' => 'Долгосрочные активы, т.р.',
            ],

            'КРАТКОСРОЧНЫЕ АКТИВЫ' => [
                '_210' => 'Запасы:',
                '_211' => 'в том числе: материалы, т.р.',
                '_213' => 'в том числе: незавершенное производство, т.р.',
                '_214' => 'в том числе: готовая продукция и товары, т.р.',
                '_215' => 'в том числе: товары отгруженные, т.р.',
                '_230' => 'Расходы будущих периодов, т.р.',
                '_240' => 'НДС по приобретенным товарам, работам, услугам, т.р.',
                '_250' => 'Краткосрочная дебиторская задолженность, т.р.',
                '_260' => 'Краткосрочные финансовые вложения, т.р.',
                '_270' => 'Денежные средства и эквиваленты, т.р.',
                '_280' => 'Прочие краткосрочные активы, т.р.',
                '_290' => 'Краткосрочные активы, т.р.',
            ],

            'КАПИТАЛ' => [
                '_410' => 'Уставный капитал, т.р.',
                '_440' => 'Резервный капитал, т.р.',
                '_450' => 'Добавочный капитал, т.р.',
                '_460' => 'Нераспределенная прибыль (непокрытый убыток), т.р.',
                '_470' => 'Чистая прибыль (убыток), т.р.',
                '_480' => 'Целевое финансирование, т.р.',
                '_490' => 'Капитал, т.р.',
            ],

            'ДОЛГОСРОЧНЫЕ ОБЯЗАТЕЛЬСТВА' => [
                '_510' => 'Долгосрочные кредиты и займы, т.р.',
                '_540' => 'Доходы будущих периодов, т.р.',
                '_590' => 'Долгосрочные долги, т.р.',
            ],

            'КРАТКОСРОЧНЫЕ ОБЯЗАТЕЛЬСТВА' => [
                '_610' => 'Краткосрочные кредиты и займы, т.р.',
                '_620' => 'Краткосрочная часть долгосрочных обязательств, т.р.',
                '_630' => 'Краткосрочная кредиторская задолженность, т.р.',
                '_631' => 'в том числе: поставщикам, подрядчикам, исполнителям, т.р.',
                '_632' => 'в том числе: по авансам полученным, т.р.',
                '_633' => 'в том числе: по налогам и сборам, т.р.',
                '_634' => 'в том числе: по социальному страхованию и обеспечению, т.р.',
                '_635' => 'в том числе: по оплате труда, т.р.',
                '_636' => 'в том числе: по лизинговым платежам, т.р.',
                '_637' => 'в том числе: собственнику имущества (учредителям, участникам), т.р.',
                '_638' => 'в том числе: прочим кредиторам, т.р.',
                '_650' => 'Доходы будущих периодов, т.р.',
                '_670' => 'Прочие краткосрочные обязательства, т.р.',
                '_690' => 'Краткосрочные долги, т.р.',
            ],

            'ИТОГО' => [
                '_700' => 'Балансовая стоимость, т.р.',
            ],
        ];
    }
